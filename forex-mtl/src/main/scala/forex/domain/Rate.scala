...
case class Rate(
  ...
  def isStale(clock: Clock): Boolean = Rate.isStale(timestamp.value, clock)
}
...
object Rate {
  ...
  val maxAgeBeforeStale = 5.minutes

  def isStale(arg: OffsetDateTime, clock: Clock): Boolean =
    Duration.between(arg.toInstant, Instant.now(clock)).toMinutes > maxAgeBeforeStale.toMinutes
